...
- **Base URL:** `https://your-app.test/api` (replace with your host)
...
The API is open to **any origin**. Browser clients on any domain can call it without proxying.
...
Because we authenticate with bearer tokens in the `Authorization` header, `supports_credentials` stays `false` — that lets us keep `allowed_origins: *` (the CORS spec forbids combining wildcard origin with credentialed requests).
...
```bash
curl -X POST https://your-app.test/api/register \
...
### `PUT /api/goals/{goal}` *(also accepts `PATCH`)*
...
```bash
# 1. Register
TOKEN=$(curl -s -X POST https://your-app.test/api/register \
  -H 'Accept: application/json' \
  -H 'Content-Type: application/json' \
...
# 2. Create a goal
GOAL_ID=$(curl -s -X POST https://your-app.test/api/goals \
...
# 3. Add a deposit
curl -s -X POST "https://your-app.test/api/goals/$GOAL_ID/deposits" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Accept: application/json' \
  -H 'Content-Type: application/json' \
...
curl -s "https://your-app.test/api/goals/$GOAL_ID" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Accept: application/json' | jq

# 5. Log out
curl -s -X POST https://your-app.test/api/logout \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Accept: application/json'
```
